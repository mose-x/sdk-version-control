...
// TestValidateCheckURL_IPLiterals covers the SSRF validation of proxy-check
// target URLs, including the net.IP-based checks for IP literals that the
// string blacklist alone misses (0.0.0.0, 169.254/16, IPv6 ULA/link-local,
// IPv4-mapped IPv6). Basic scheme/private cases are covered by
// TestValidateCheckURL in helpers_test.go.
func TestValidateCheckURL_IPLiterals(t *testing.T) {
	tests := []struct {
		url     string
		wantErr bool
	}{
...
func TestCheckProxy_RejectsInternalTargets(t *testing.T) {
	app := &App{settings: config.NewSettingsManager(t.TempDir())}
	for _, target := range []string{"http://127.0.0.1/", "http://10.0.0.1/", "ftp://example.com"} {
		if err := app.CheckProxy(target); err == nil {
...
// TestBuildProxyTransport verifies the update checker's transport shares the
...
func TestBuildProxyTransport(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			app := &App{settings: sm}
			transport := app.buildProxyTransport()
